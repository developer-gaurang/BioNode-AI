    ...
    return Stack(
      children: [
        ...
        Positioned(
          top: -150,
          left: -150,
          child: _buildOrb(const Color(0xFFA855F7), 400, 0.15),
        ),
        Positioned(
          bottom: -200,
          right: -100,
          child: _buildOrb(const Color(0xFFC084FC), 500, 0.1),
        ),
        Positioned.fill(
...
class _SpringButtonState extends State<SpringButton>
  ...
  void initState() {
    ...
    _scaleAnimation = Tween<double>(begin: 1.0, end: 0.92).animate(
      CurvedAnimation(
        parent: _controller,
        curve: const ElasticOutCurve(0.9), // Extremely satisfying spring
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          suffixIcon: isPassword
              ...
              : null,
          hintText: hint,
          hintStyle: GoogleFonts.inter(color: Colors.white30, fontSize: 18),
  ...
  Future<void> _fetchRealTelemetry() async {
    ...
    try {
      ...
      final String prompt =
          ...
          '"summary": "Weather impact right now in 2 sentences", "precaution": "2 vital precautions for today", '
          '"forecast": "1 short summary about how weather will change in next 4-5 hours", "isHighRisk": false}';

      // 3. Analyze with Gemini (Stable v1 Endpoint)
      const String model = 'gemini-1.5-flash';
      final String apiKey = dotenv.env['GEMINI_API_KEY'] ?? 'AIzaSyBYKIwSztuPyiovRLnveWt6T821SPDSz40';
      final String url = 'https://generativelanguage.googleapis.com/v1/models/$model:generateContent?key=$apiKey';
      ...
      if (geminiRes.statusCode == 200) {
        ...
        String rawText = geminiData['candidates'][0]['content']['parts'][0]['text'] ?? '';
        ...
        if (mounted) setState(() => _geminiError = 'AI Sync Error (${geminiRes.statusCode})');
      }
    } finally {
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildLegend(
                          const Color(0xFFFF2A5F),
                          'CRITICAL (RED)',
                          'Active Heat-Stroke risk zone. Route discouraged.',
                        ),
                        const SizedBox(height: 12),
                        _buildLegend(
                          const Color(0xFFFF9100),
...
class PricingScreen extends StatelessWidget {
  ...
  Widget _buildFeature(String text) {
    return Padding(
      ...
      child: Row(
        children: [
          const Icon(Icons.check_circle, color: Color(0xFFA855F7), size: 28),
          const SizedBox(width: 24),
          Expanded(
            child: Text(
              text,
...
class _CrashGuardTabState extends State<CrashGuardTab>
  ...
  Future<void> _loadContacts() async {
    final prefs = await SharedPreferences.getInstance();
    final raw = prefs.getStringList('crash_contacts_${widget.userAlias}') ?? [];
    setState(() {
      _contacts.clear();
  ...
  Future<void> _saveContacts() async {
    ...
    await prefs.setStringList(
      'crash_contacts_${widget.userAlias}',
  ...
  Future<void> _sendFirebaseAlert() async {
    try {
      await FirebaseFirestore.instance.collection('crash_alerts').add({
        ...
        'status': 'ACTIVE',
        'message':
            'SOS! ${widget.userName} may have been in an accident near (${_userLat.toStringAsFixed(4)}, ${_userLon.toStringAsFixed(4)}). Immediate assistance needed!',
      });
    } catch (e) {
